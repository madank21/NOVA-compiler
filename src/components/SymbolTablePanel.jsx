    ...
    <div className="flex flex-col h-full bg-[#1e1e1e] p-4 text-xs font-mono">
      <div className="flex items-center justify-between mb-4 border-b border-[#333333] pb-2">
        <div className="flex items-center space-x-2">
          <Database aria-hidden="true" className="w-4 h-4 text-teal-400" />
          <span className="font-bold text-gray-200">Phase 03: Semantic Analysis &amp; Symbol Table</span>
        </div>
        <span className="text-[11px] text-gray-400">{symbolTable.length} symbols · scoped layout with slot addresses</span>
      </div>

      {symbolTable.length === 0 ? (
        <div className="flex-1 flex items-center justify-center text-gray-600 text-[11px]">
          Compile a program to build its symbol table.
        </div>
      ) : (
          ...
          <table className="w-full text-left border-collapse">
            <thead className="sticky top-0">
              <tr className="bg-[#252526] text-gray-400 border-b border-[#333333]">
                <th scope="col" className="p-2.5 border-r border-[#333333]">Scope</th>
                <th scope="col" className="p-2.5 border-r border-[#333333]">Symbol Name</th>
                <th scope="col" className="p-2.5 border-r border-[#333333]">Kind</th>
                <th scope="col" className="p-2.5 border-r border-[#333333]">Type</th>
                <th scope="col" className="p-2.5 border-r border-[#333333]">Slot Address</th>
                <th scope="col" className="p-2.5">Params</th>
              </tr>
            </thead>
            <tbody>
              {symbolTable.map((sym, idx) => (
                <tr key={idx} className="border-b border-[#2a2a2a] hover:bg-[#252526] transition">
                  ...
                  <td className="p-2.5 text-gray-400">{sym.params === -1 ? 'variadic' : sym.params}</td>
                </tr>
              ))}
            </tbody>
          </table>
