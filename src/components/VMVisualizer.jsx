...
import { Play, Pause, SkipForward, RotateCcw, Cpu, HardDrive, Layers, Terminal } from 'lucide-react';
...
export default function VMVisualizer({ vmTrace, onStepChange }) {
  ...
  useEffect(() => {
    if (onStepChange && currentStep.line) {
  ...
  // Derive call stack activation frames for visualization
  const callStackFrames = [
    { frame: 0, func: 'main()', retAddr: '0x0000', scope: 'Global' }
  ];
  if (currentStep.instruction.includes('CALL') || currentStepIdx > 3) {
    callStackFrames.push({ frame: 1, func: 'factorial(n=5)', retAddr: `0x000${currentStep.pc}`, scope: 'Local' });
  }

  return (
    <div className="flex flex-col h-full bg-[#1e1e1e] p-4 text-xs font-mono">
      {/* Playback Control Bar */}
      <div className="flex items-center justify-between mb-4 bg-[#252526] border border-[#333333] p-3 rounded-lg">
        <div className="flex items-center space-x-3">
          <button
            onClick={() => setIsPlaying(!isPlaying)}
            className="bg-blue-600 hover:bg-blue-500 text-white p-2 rounded-md transition shadow-md"
          >
          ...
          <button
            ...
            className="bg-[#2d2d2d] hover:bg-[#383838] text-gray-300 px-3 py-1.5 rounded border border-[#3c3c3c] transition disabled:opacity-40"
          >
            Step Back
          </button>
        ...
        {/* Speed Slider */}
        <div className="flex items-center space-x-3">
          <span className="text-gray-400 text-[11px]">Speed: {speedMs}ms</span>
          <input
            type="range"
            min="100"
            max="1000"
      ...
      <div className="flex-1 grid grid-cols-4 gap-3 overflow-hidden">
        ...
        <div className="flex flex-col bg-[#252526] border border-[#333333] rounded-lg p-3 overflow-auto">
          <div className="flex items-center justify-between border-b border-[#333333] pb-2 mb-3 text-indigo-400 font-bold">
            ...
            <span className="text-[10px] text-gray-500">{callStackFrames.length} Frames</span>
          </div>

          <div className="space-y-2 overflow-auto flex-1">
            {callStackFrames.map((f, idx) => (
              <div key={idx} className="bg-[#1e1e1e] border border-indigo-500/30 p-2 rounded">
                ...
                <div className="text-[10px] text-gray-500 mt-1">Ret Addr: {f.retAddr}</div>
              </div>
            ))}
          </div>
        </div>
            ...
            {currentStep.variables.length === 0 ? (
              ...
              currentStep.variables.map((v, idx) => (
                ...
                >
                  ...
                  <span className="text-amber-400 font-bold">{v.value}</span>
                </div>
              ))
            )}
          </div>
