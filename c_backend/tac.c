...
static void binding_push(G *g) {
    if (g->bindingDepth >= g->bindingCapacity) {
        g->bindingCapacity = g->bindingCapacity ? g->bindingCapacity * 2 : 16;
        g->bindingScopes = (BindingScope *)xrealloc(g->bindingScopes,
            sizeof(BindingScope) * (size_t)g->bindingCapacity);
    }
    memset(&g->bindingScopes[g->bindingDepth], 0, sizeof(BindingScope));
    g->bindingDepth++;
}

static void binding_pop(G *g) {
    if (g->bindingDepth <= 0) return;
    BindingScope *scope = &g->bindingScopes[--g->bindingDepth];
    free(scope->names);
    free(scope->recs);
    memset(scope, 0, sizeof(*scope));
}

static void binding_add(G *g, const char *name, SymRec *rec) {
    if (g->bindingDepth <= 0 || !rec) return;
    BindingScope *scope = &g->bindingScopes[g->bindingDepth - 1];
    if (scope->count >= scope->capacity) {
        scope->capacity = scope->capacity ? scope->capacity * 2 : 8;
        scope->names = (char (*)[256])xrealloc(scope->names,
            sizeof(char[256]) * (size_t)scope->capacity);
        scope->recs = (SymRec **)xrealloc(scope->recs,
            sizeof(SymRec *) * (size_t)scope->capacity);
    }
    strncpy(scope->names[scope->count], name, 255);
    scope->names[scope->count][255] = '\0';
    scope->recs[scope->count] = rec;
    scope->count++;
}

static SymRec *binding_find(G *g, const char *name) {
    for (int d = g->bindingDepth - 1; d >= 0; d--) {
        BindingScope *scope = &g->bindingScopes[d];
        for (int i = scope->count - 1; i >= 0; i--) {
            if (strcmp(scope->names[i], name) == 0) return scope->recs[i];
        }
    }
    return NULL;
}

static SymRec *binding_for_node(G *g, NovaNode *node) {
    if (!node || !node->has_binding) return NULL;
    if (node->binding_is_global) {
        for (int i = 0; i < g->sem->globals.count; i++) {
            SymRec *r = &g->sem->globals.items[i];
            if (r->offset == node->binding_offset && strcmp(r->name, node->identifier) == 0) return r;
        }
        return NULL;
    }
    FuncDef *f = g->currentFunc ? sem_find_function(g->sem, g->currentFunc) : NULL;
    if (!f) return NULL;
    for (int i = 0; i < f->frame.count; i++) {
        SymRec *r = &f->frame.items[i];
        if (!r->isGlobal && r->offset == node->binding_offset && strcmp(r->name, node->identifier) == 0) return r;
    }
    return NULL;
}

/* Resolve metadata in the active lexical scope, then at file scope. */
static SymRec *find_any_symbol(G *g, const char *name) {
    if (!g->generatingGlobalInits) {
        SymRec *r = binding_find(g, name);
        if (r) return r;
    }
    return sem_find_global(g->sem, name);
}

static const char *storage_name_of(G *g, const char *name) {
    SymRec *r = find_any_symbol(g, name);
    return (r && r->storage[0]) ? r->storage : name;
}

static const char *declaration_storage(G *g, NovaNode *node) {
    if (g->generatingGlobalInits) return node->identifier;
    SymRec *r = binding_for_node(g, node);
    return (r && r->storage[0]) ? r->storage : node->identifier;
}
...
static void gen_index_addr(G *g, NovaNode *node, char out[32]) {
    ...
    if (base->type == NODE_IDENTIFIER) {
        ...
        if (rec && rec->is_array && !rec->isParam) {
            ...
            strncpy(baseName, storage_name_of(g, base->identifier), 63);
            baseName[63] = '\0';
        ...
        emit(g, "+", t, storage_name_of(g, base->identifier), idx.place, node->line);
        strcpy(out, t);
        return;
    }
...
typedef struct { char place[32]; char fieldType[160]; int isArray; int ok; } MemberAddr;
            ...
            if (st) {
                for (int i = 0; i < st->field_count; i++) {
                    if (strcmp(st->fields[i].name, node->identifier) == 0) {
                        ...
                        emit(g, "ADDR", t, storage_name_of(g, base->identifier), off, node->line);
                        strcpy(ma.place, t);
                        ...
                        ma.isArray = st->fields[i].is_array;
                        ma.ok = 1;
                        return ma;
                    }
                }
...
static const char *normalized_value_type(const char *type) {
    if (type && (strcmp(type, "double") == 0 || strcmp(type, "float") == 0)) return "double";
    if (type && type[0] && type[strlen(type) - 1] == '*') return "ptr";
    return "int";
}

static const char *dereferenced_value_type(const char *type) {
    if (!type) return "int";
    size_t n = strlen(type);
    if (n == 0 || type[n - 1] != '*') return "int";
    if (n >= 2 && type[n - 2] == '*') return "ptr";
    if ((n == 7 && strncmp(type, "double", 6) == 0) ||
        (n == 6 && strncmp(type, "float", 5) == 0)) return "double";
    return "int";
}

/* Resolve member metadata without emitting TAC. */
static StructField *member_field_info(G *g, NovaNode *node) {
    if (!node || node->type != NODE_MEMBER) return NULL;
    NovaNode *base = node->children[0];
    StructDef *st = NULL;
    if (base && base->type == NODE_IDENTIFIER) {
        SymRec *rec = find_any_symbol(g, base->identifier);
        if (rec && strncmp(rec->type, "struct ", 7) == 0) st = sem_find_struct(g->sem, rec->type + 7);
    } else if (base && base->type == NODE_MEMBER) {
        StructField *parent = member_field_info(g, base);
        if (parent && !parent->is_array && strncmp(parent->type, "struct ", 7) == 0)
            st = sem_find_struct(g->sem, parent->type + 7);
    }
    if (!st) return NULL;
    for (int i = 0; i < st->field_count; i++) {
        if (strcmp(st->fields[i].name, node->identifier) == 0) return &st->fields[i];
    }
    return NULL;
}

static const char *index_element_type(G *g, NovaNode *base) {
    if (!base) return "int";
    if (base->type == NODE_IDENTIFIER) {
        SymRec *rec = find_any_symbol(g, base->identifier);
        if (rec) {
            if (rec->is_array) return normalized_value_type(rec->type);
            return dereferenced_value_type(rec->type);
        }
    } else if (base->type == NODE_MEMBER) {
        StructField *field = member_field_info(g, base);
        if (field) {
            if (field->is_array) return normalized_value_type(field->type);
            return dereferenced_value_type(field->type);
        }
    }
    return "int";
}
...
/* lvalue addressing */
typedef struct { int valid; int direct; char name[64]; char place[32]; char type[16]; } LValue;
...
static void gen_assignment(G *g, NovaNode *node, Place *out) {
    ...
    if (lv.direct) {
        ...
        const char *valueType = lv.type[0] ? lv.type : "int";
        char t[32]; new_temp(g, valueType, t);
        emit(g, "LOAD_PTR", t, lv.place, "", node->line);
        strcpy(oldV.place, t);
        strcpy(oldV.type, valueType);
    }
    Place rhs = gen_expr(g, node->children[1]);
...
static LValue lvalue_addr(G *g, NovaNode *node) {
    ...
    if (node->type == NODE_IDENTIFIER) {
        ...
        strncpy(lv.name, storage_name_of(g, node->identifier), sizeof(lv.name) - 1);
        return lv;
    }
    if (node->type == NODE_INDEX) {
        lv.valid = 1; lv.direct = 0;
        strncpy(lv.type, index_element_type(g, node->children[0]), sizeof(lv.type) - 1);
    ...
    if (node->type == NODE_MEMBER) {
        ...
        if (ma.ok) {
            lv.valid = 1; lv.direct = 0; strcpy(lv.place, ma.place);
            strncpy(lv.type, ma.isArray ? "ptr" : normalized_value_type(ma.fieldType), sizeof(lv.type) - 1);
        }
        return lv;
    }
    if (node->type == NODE_UNARY_OP && strcmp(node->op, "*") == 0) {
        ...
        lv.valid = 1; lv.direct = 0;
        strncpy(lv.type, p.type[0] ? dereferenced_value_type(p.type) : "int", sizeof(lv.type) - 1);
...
static void gen_call(G *g, NovaNode *node, Place *out) {
    ...
    FuncDef *fn = sem_find_function(g->sem, node->identifier);
    const char *declaredType = fn ? fn->type_name : sem_builtin_type(node->identifier);
    const char *resultType = normalized_value_type(declaredType ? declaredType : "int");
    char t[32]; new_temp(g, resultType, t);
    char nargs[16];
    snprintf(nargs, sizeof(nargs), "%d", node->child_count);
    emit(g, "CALL", t, node->identifier, nargs, node->line);
    memset(out, 0, sizeof(Place));
    strcpy(out->place, t);
    strcpy(out->type, resultType);
}
...
static Place gen_expr(G *g, NovaNode *node) {
    ...
    if (node->type == NODE_IDENTIFIER) {
        ...
        if (rec && rec->is_array) {
            ...
            if (rec->isParam) {
                Place p; memset(&p, 0, sizeof(p));
                strncpy(p.place, storage_name_of(g, node->identifier), sizeof(p.place) - 1);
                strcpy(p.type, "ptr");
                return p;
            }
            char t[32]; new_temp(g, "ptr", t);
            emit(g, "ADDR", t, storage_name_of(g, node->identifier), "0", node->line);
    ...
    if (node->type == NODE_UNARY_OP) {
        ...
        if (strcmp(node->op, "&") == 0) {
            ...
            if (inner->type == NODE_IDENTIFIER) {
                char t[32]; new_temp(g, "ptr", t);
                emit(g, "ADDR", t, storage_name_of(g, inner->identifier), "0", inner->line);
                Place p; memset(&p, 0, sizeof(p)); strcpy(p.place, t); strcpy(p.type, "ptr");
                return p;
            }
            ...
            if (inner->type == NODE_MEMBER) {
                MemberAddr ma = gen_member_addr(g, inner);
                if (ma.ok) {
                    Place p; memset(&p, 0, sizeof(p)); strcpy(p.place, ma.place); strcpy(p.type, "ptr");
                    return p;
                }
            }
            char t[32]; new_temp(g, "ptr", t);
    ...
    if (node->type == NODE_INDEX) {
        ...
        const char *valueType = index_element_type(g, node->children[0]);
        char t[32]; new_temp(g, valueType, t);
        emit(g, "LOAD_PTR", t, addr, "", node->line);
        Place p; memset(&p, 0, sizeof(p)); strcpy(p.place, t); strcpy(p.type, valueType);
        return p;
    }
    if (node->type == NODE_MEMBER) {
        MemberAddr ma = gen_member_addr(g, node);
        ...
        if (ma.isArray) {
            Place p; memset(&p, 0, sizeof(p));
            strcpy(p.place, ma.place); strcpy(p.type, "ptr");
            return p;
        }
        const char *valueType = normalized_value_type(ma.fieldType);
        char t[32]; new_temp(g, valueType, t);
...
/* Expand a struct into ordered leaf slots while retaining array-subobject
 * boundaries. A string literal consumes an entire char-array field before the
 * next initializer advances to the following field. */
typedef struct { int offset; int arrayStart; int arraySize; } InitLeaf;

static void struct_init_leaves(SemResult *sem, const char *structName,
                               InitLeaf *out, int *count, int base) {
    ...
    for (int i = 0; i < st->field_count && *count < 1024; i++) {
        ...
        if (strncmp(f->type, "struct ", 7) == 0) {
            if (!f->is_array) struct_init_leaves(sem, f->type + 7, out, count, base + f->offset);
            continue;
        }
        if (f->is_array) {
            int start = base + f->offset;
            for (int k = 0; k < f->size && *count < 1024; k++) {
                out[*count].offset = start + k;
                out[*count].arrayStart = start;
                out[*count].arraySize = f->size;
                (*count)++;
            }
            continue;
        }
        out[*count].offset = base + f->offset;
        out[*count].arrayStart = -1;
        out[*count].arraySize = 0;
        (*count)++;
    }
}
...
static void emit_var_decl_init(G *g, NovaNode *node) {
    const char *declPlace = declaration_storage(g, node);
    if (node->is_array) {
        ...
        if (inits == 1 && node->children[start]->type == NODE_STRING_LITERAL) {
            ...
            for (int i = 0; i < n && i < cnt; i++) {
                ...
                emit(g, "IDX_ADDR", idxT, declPlace, istr, node->line);
                char val[32];
    ...
    if (node->child_count > 0 && strncmp(node->type_name, "struct ", 7) == 0) {
        InitLeaf leaves[1024]; int nleaves = 0;
        struct_init_leaves(g->sem, node->type_name + 7, leaves, &nleaves, 0);
        int cursor = 0;
        for (int i = 0; i < node->child_count && cursor < nleaves; i++) {
            NovaNode *init = node->children[i];
            InitLeaf *leaf = &leaves[cursor];
            if (init->type == NODE_STRING_LITERAL && leaf->arraySize > 0 &&
                leaf->offset == leaf->arrayStart) {
                const char *str = init->string_val;
                int slen = (int)strlen(str);
                int copied = slen + 1 < leaf->arraySize ? slen + 1 : leaf->arraySize;
                for (int k = 0; k < copied; k++) {
                    char t[32], off[32], val[32]; new_temp(g, "ptr", t);
                    snprintf(off, sizeof(off), "%d", leaf->arrayStart + k);
                    snprintf(val, sizeof(val), "%d", k < slen ? (unsigned char)str[k] : 0);
                    emit(g, "ADDR", t, declPlace, off, node->line);
                    emit(g, "STORE_PTR", "", t, val, node->line);
                }
                for (int k = copied; k < leaf->arraySize; k++) {
                    char t[32], off[32]; new_temp(g, "ptr", t);
                    snprintf(off, sizeof(off), "%d", leaf->arrayStart + k);
                    emit(g, "ADDR", t, declPlace, off, node->line);
            ...
            Place v = gen_expr(g, init);
            char t[32], off[32]; new_temp(g, "ptr", t);
            snprintf(off, sizeof(off), "%d", leaf->offset);
            emit(g, "ADDR", t, declPlace, off, node->line);
            emit(g, "STORE_PTR", "", t, v.place, node->line);
            cursor++;
        }
        for (; cursor < nleaves; cursor++) {
            char t[32], off[32]; new_temp(g, "ptr", t);
            snprintf(off, sizeof(off), "%d", leaves[cursor].offset);
            emit(g, "ADDR", t, declPlace, off, node->line);
            emit(g, "STORE_PTR", "", t, "0", node->line);
        }
        return;
    ...
    if (node->child_count > 0) {
        Place v = gen_expr(g, node->children[0]);
        emit(g, "=", declPlace, v.place, "", node->line);
    }
}
...
static void bind_declaration(G *g, NovaNode *node) {
    SymRec *rec = binding_for_node(g, node);
    if (rec) binding_add(g, node->identifier, rec);
}
...
static void gen_stmt(G *g, NovaNode *node) {
    ...
    if (node->type == NODE_VAR_DECL) { bind_declaration(g, node); gen_var_decl(g, node); return; }
...
TacResult *nova_gen_tac(NovaNode *ast, SemResult *sem, DiagList *diags) {
    ...
    for (int i = 0; i < ast->child_count; i++) {
        ...
        g.currentFunc = top->identifier;
        binding_push(&g);
        for (int c = 0; c < top->child_count; c++) {
            if (top->children[c]->type == NODE_PARAMETER) bind_declaration(&g, top->children[c]);
        }
    ...
    while (g.bindingDepth > 0) binding_pop(&g);
    free(g.bindingScopes);
    free(g.labelNames);
    free(g.labelIds);
    return g.r;
}
